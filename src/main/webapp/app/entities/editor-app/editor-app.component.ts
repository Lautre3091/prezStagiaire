...
@Component({
    selector: 'jhi-editor-app',
    templateUrl: './editor-app.component.html'
})
export class EditorAppComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.editorService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<EditorApp[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/editor-app'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/editor-app', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: EditorApp) {
        return item.id;
    }
